...
import { Position, ProductType, OrderType } from "@/lib/types/database";
...
export const OrderForm: React.FC<OrderFormProps> = ({
  ...
  const heldQuantity =
    positions.find(
      (position) =>
        position.symbol === symbol && position.product_type === productType,
    )?.quantity ?? 0;
  const hasSufficientHolding = side === "BUY" || quantity <= heldQuantity;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (side === "BUY") {
      if (
        validation.data.stopLossPrice != null &&
        validation.data.stopLossPrice >= currentMarketPrice
      ) {
        setErrorMsg("Stop loss must be below the current market price");
        return;
      }
      if (
        validation.data.targetPrice != null &&
        validation.data.targetPrice <= currentMarketPrice
      ) {
        setErrorMsg("Target price must be above the current market price");
        return;
      }
    }

    if (side === "BUY" && !hasSufficientFunds) {
      setErrorMsg(
  ...
  return (
    <div className="flex flex-col w-full bg-[#161b22] border border-[#21262d] rounded-lg p-4 text-slate-200">
      ...
      <div className="flex items-center justify-between border-b border-[#21262d] pb-3 mb-4">
        ...
        <div className="flex gap-1 rounded border border-[#30363d] bg-[#0d1117] p-1">
